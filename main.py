...
def _run_db_setup() -> None:
    """
    Chạy tự động kịch bản `setup_database.py` trong một luồng nền (background thread).
    Nhiệm vụ:
    - Bật service SQL Server nếu đang tắt.
    - Tạo database `He_Thong_Du_Doan_Thuoc` nếu chưa có.
    - Tạo tất cả các bảng và user mặc định (admin/expert/user).
    - Đồng bộ JSON file vào database.
    Kết quả log sẽ được dẫn thẳng ra console mà không làm đứng giao diện UI.
    """
    setup_path = ROOT / "src" / "scripts" / "database" / "setup_database.py"
...
def start_api(host: str = DEFAULT_API_HOST, port: int = DEFAULT_API_PORT, reload: bool = False) -> threading.Thread:
    """
    Hàm khởi động Backend (FastAPI).
    - Tạo một luồng (thread) mới và chạy server Uvicorn.
    - Lắng nghe các yêu cầu API (như /predict/..., /auth/...) từ Frontend.
    """
...
def start_streamlit(
    ...
    """
    Hàm khởi động giao diện người dùng (Frontend) bằng framework Streamlit.
    - Chạy lệnh `streamlit run src/frontend/app_modern.py`.
    - Ứng dụng Modern UI sẽ giao tiếp với Backend qua cổng 8000 thông qua class `ApiClient`.
    """
