...
LOGS_DIR = _PROJECT_ROOT / "train_logs"
...
class TrainLogger:
    ...
    def _print_summary(self, r: TrainRecord) -> None:
        ...
        lines += [
            f"{'─'*65}",
            f"  Saved to : train_logs/{r.dataset}/runs.jsonl",
            f"             train_logs/{r.dataset}/runs_table.csv",
            f"{'='*65}\n",
        ]
        for line in lines:
            _safe_print(line)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "leaderboard":
        ...
        print("Cách dùng:")
        print("  python train_logger.py leaderboard")
        print("  python train_logger.py show B-dataset")
