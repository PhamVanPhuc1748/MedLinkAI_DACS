...
import sys
try:
    from . import fuzzy_layer, gcn_flow, gnn_algorithm
except ImportError:
    import fuzzy_layer, gcn_flow, gnn_algorithm

# Hack để hỗ trợ load model .pth được train bằng script ở thư mục gốc (với các module top-level)
sys.modules['mo_hinh_ai'] = sys.modules.get(__name__)
sys.modules['fuzzy_layer'] = sys.modules.get(fuzzy_layer.__name__)
sys.modules['gcn_flow'] = sys.modules.get(gcn_flow.__name__)
sys.modules['gnn_algorithm'] = sys.modules.get(gnn_algorithm.__name__)
