...
# ── Đường dẫn: src/training/ → project root là 2 cấp trên ─────────────────
...
# Import TrainLogger — log mọi trial vào src/training/train_logs/
try:
    from train_logger import TrainLogger
    _LOGGER_AVAILABLE = True
except ImportError:
    try:
        from src.training.train_logger import TrainLogger
        _LOGGER_AVAILABLE = True
    except ImportError:
        _LOGGER_AVAILABLE = False

# ── Thư mục lưu lịch sử ────────────────────────────────────────────────────
LICH_SU_DIR = _THIS_DIR / "lich_su_thong_so_train"
